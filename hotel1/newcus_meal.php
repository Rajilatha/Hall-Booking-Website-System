...
<div class="header col-12">
	<h1 style="font-family:Papyrus">JK ONLINE HALL BOOKING <?php  if (isset($_SESSION['CustomerID'])) {
		...
		<nav class="navbar navbar-light" style="background-color:#16242D;">
										...
										<br>
	<?php
echo "<form action='cart.php' method='post' id='id'>";
$cid = $_POST['cid'];
...
?>
		...
		<div class="container">
			...
			<?php
				...
				?>
			<div class="col-md-4">
				<form method="post" action="newcus_meal.php?action=add&id=<?php echo $row["meal_id"]; ?>">
...
<?php
echo "<form action='chat.php' method='post' id='id'>";
$cid = $_POST['cid'];

echo "<input type='hidden' name='cid' value=$cid color='white' class='form-control' />";
echo" <input type='submit' value='Chat' name='submit'>";
echo "</form>";	
?>
			<form action="newcus_meal.php" method="post" style="align:left;">
			<div class="row">
      <div class="col-25">
	...
	if($_GET["action"] == "delete")
	{
		foreach($_SESSION["shopping_cart"] as $keys => $values)
		{
			if($values["item_id"] == $_GET["id"])
			{
				...
				echo '<script>window.location="newcus_meal.php"</script>';
			}
		}
	}
}
