...
<?php
echo "<form action='cart2.php' method='post' id='id'>";
$cid = $_POST['cid'];

echo "<input type='hidden' name='cid' value=$cid class='form-control' />";
echo" <input type='submit' value='Cart' name='submit'>";
echo "</form>";
?>
		...
		<div class="container">
			...
			<?php
				...
				?>
			<div class="col-md-4">
				<form method="post" action="newcus_mealp.php?action=add&id=<?php echo $row["meal_id"]; ?>">
...
echo "<input type='text' name='cid' value=$cid class='form-control' />";
...
?>
			...
			<form action="addcartg.php" method="POST">
	...
	<?php
echo "<form action='chat2.php' method='post' id='id'>";
$cid = $_POST['cid'];

echo "<input type='hidden' name='cid' value=$cid class='form-control' />";
echo" <input type='submit' value='Chat' name='submit'>";
echo "</form>";	
?>
			<form action="newcus_mealp.php" method="post" style="align:left;">
			<div class="row">
      <div class="col-25">
        <label for="quantity">Booking Id</label>
		</div>
		  <div class="col-75">
		<?php
...
if(isset($_POST["add_to_cart"]))
{
	...
	$sql = "INSERT INTO cartp (CustomerID,meal_id,quantity,booking_id) VALUES ('" . $CustomerID . "','" . $item_id . "','" . $item_quantity . "','" . $cid . "')";
    $result = mysqli_query($con, $sql);
	if ($result > 0) {
				echo "<form action='pay/pricing.php' method='post' id='id'>";
	...
	if($_GET["action"] == "delete")
	{
		foreach($_SESSION["shopping_cart"] as $keys => $values)
		{
			if($values["item_id"] == $_GET["id"])
			{
				...
				echo '<script>window.location="newcus_mealp.php"</script>';
			}
		}
	}
}
...
$quantity=filter_input(INPUT_POST,'quantity');
		...
		if($quantity!==""||$custom!==""){
		$sql="INSERT INTO cartp ( CustomerID,quantity, meal_id,booking_id) VALUES ('$CustomerID','$quantity','$custom','$cid')";

		if($con->query($sql))
		{
			echo "<h1> Successfully Add Custom meal</h1>  ";
		}
		else{
			echo "";
		}
