					 ...
					 function conDelete(y) 
{
 ...
 if(x) {
 	...
 	window.location.replace("room_delete.php?id="+y);
			 ...
			 <div id="navcontent">
				...
				<!--searching bar-->
					...
					<div class="col-sm-6" style="padding-right:33px;">
						<button style="float:right;" class="btn btn-dark"type="button" onClick= "window.location.href ='addroom_details.php';" ><i class=></i>Add Room Details</button>
					</div>
									...
									<tr>
										...
										<th></th>		
											...
											if(isset($_POST["submit"])) {
												...
												$sql = "SELECT * FROM rooms WHERE room_no LIKE '%{$_POST["keyname"]}%' OR room_type LIKE '%{$_POST["keyname"]}%' OR room_category LIKE '%{$_POST["keyname"]}%'OR cost LIKE '%{$_POST["keyname"]}%'"; 
											...
											if ($result -> num_rows>0 ) {
											  ...
											  while($row = $result->fetch_assoc()) {
													...
													echo "<tr>";
														  ...
														  echo "<td>"; echo $row['room_category'];  echo "</td>";
														  echo "<td>"; echo $row['cost'];  echo "</td>";
														   echo "<td>"; echo "<img src=Images/".$row['image_link'].">"; echo "</td>";
														  echo "<td>";
														  echo "<button type='button' class='btn btn-danger'> <a style='color:white;text-decoration:none;' onClick='conDelete({$row["room_no"]})'>Delete </a></button>";
														  echo "</td>";
