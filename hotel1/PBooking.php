...
<div class="container">
  <form action="pBooking.php" method="post">
    ...
    <div class="row">
      ...
      <div class="col-75">
      <input type="text" id="hallname" name="hallname" value="Platinum" required>
...
if(isset($_POST['submit'])){
	...
	$room_no=filter_input(INPUT_POST,'room_no');
		...
		$query=mysqli_query($con,("SELECT * FROM platinum WHERE Date='".$Date."' AND Time='".$Time."'AND hallname='".$hallname."'"));  
    ...
    if($hallname!==""&&$Date!==""&&$Time!==""&&$people!==""&&$decid!==""&&$room_no!==""&&$package_id!==""){
		$sql="INSERT INTO platinum (CustomerID,hallname, Date, Time, people,decoration_id,room_no,package_id) VALUES ('$CustomerID','$hallname','$Date','$Time','$people','$decid','$room_no','$package_id')";
		if($con->query($sql))
		{
			echo "<h1>Successfully submitted<h1>";
			
										$sql = "SELECT * FROM platinum WHERE Date='".$Date."' AND Time='".$Time."'AND hallname='".$hallname."'";
if($result = mysqli_query($con, $sql)){
    if(mysqli_num_rows($result) > 0){
      
        while($row = mysqli_fetch_array($result)){
            echo "<form action='newcus_mealp.php' method='post' id='id'>";
