...
<div class="container">
  <form action="gBooking.php" method="post">
    ...
    <div class="row">
      ...
      <div class="col-75">
      <input type="text" id="hallname" name="hallname" value="Gold" required>
...
if(isset($_POST['submit'])){
    ...
    if($hallname!==""&&$Date!==""&&$Time!==""&&$people!==""&&$decid!==""&&$room_no!==""&&$package_id!==""){
		$sql="INSERT INTO gold (CustomerID,hallname, Date, Time, people,decoration_id,room_no,package_id) VALUES ('$CustomerID','$hallname','$Date','$Time','$people','$decid','$room_no','$package_id')";
		if($con->query($sql))
		{
			echo "<h1>Successfully submitted<h1>";
			
										$sql = "SELECT * FROM gold WHERE Date='".$Date."' AND Time='".$Time."'AND hallname='".$hallname."'";
if($result = mysqli_query($con, $sql)){
    if(mysqli_num_rows($result) > 0){
      
        while($row = mysqli_fetch_array($result)){
            echo "<form action='newcus_mealg.php' method='post' id='id'>";
